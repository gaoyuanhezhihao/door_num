...
/* chane the speed of car.
 * -----------------------
 *  @perc_speed: percent of speed changed to.New speed = perc_speed * std speed.
 *  ------------------------------
 *  return: success or fail.
 * */
bool change_speed(double perc_speed, int sclient) {
  string cmd_2_car("p\n");
  cmd_2_car += to_string(perc_speed);

  string ack_msg("p_ack\n");
  char sock_rcv_buf[90] = {0};
  int rcv_byte_count = 0;
  do{
      sock_rcv_buf[0] = 0;
      send(sclient, cmd_2_car.c_str(), cmd_2_car.size(), 0);
      rcv_byte_count = recv(sclient, sock_rcv_buf, 90, 0);
      sock_rcv_buf[rcv_byte_count] = 0;
  }while(0 !=strcmp(sock_rcv_buf, ack_msg.c_str()));
  return true;
}
